...
public class Reporting {
	...
	public static void update(String update) {
		...
		try {
			Statement stmt = connection.createStatement();
			rset = stmt.executeUpdate(update);
		} catch (SQLException e) {
			System.out.println("Get Data Failed! Check output console");
			e.printStackTrace();
		}
	...
	public static void main(String[] args) {
		...
		case 1:
			try {
				...
				ResultSet rset = query("SELECT * FROM CUSTOMER WHERE customerID = " + Integer.toString(i));
		...
		case 2:
			try {
				...
				ResultSet rset = query("SELECT * FROM GUIDE WHERE DriverLicense = " + Long.toString(i));
		...
		case 3:
			try {
				...
				Scanner scan = new Scanner(System.in);
				long i = scan.nextLong();
				ResultSet rset = query("SELECT * FROM BookedTour_AD WHERE BookedTourID = " + Long.toString(i));
				...
				while (rset.next() && rset != null) {
					...
					ResultSet secondRes = query("SELECT firstname,lastname,age FROM customer c, (SELECT travelingwithid FROM travelingwith WHERE customerid = "+ custID +") t WHERE c.customerid = t.travelingwithid");
		...
		case 4:
			...
			Scanner scan = new Scanner(System.in);
			long i = scan.nextLong();
			System.out.println("Enter New Vehicle License Plate: ");
			long j = scan.nextLong();
			
		    update("UPDATE BookedTour SET licensePlate="+j+" WHERE bookedTourID = "+i);
			break;
...
public class SomeDatabase {
	...
	public SomeDatabase(String username, String password){
